...
const pages = dv.pages('"20-Проекты"')
  .where(page => isContentPage(page, ["Проекты"]))
  .array()
  .sort((a, b) => a.file.name.localeCompare(b.file.name, "ru"));

const groupsMap = new Map();
for (const item of pages) {
  const rel = (item.file.folder || "").replace("20-Проекты/", "");
...
hero.createEl("div", { cls: "ud-note-eyebrow", text: "Проектный обзор" });
hero.createEl("h1", { cls: "ud-note-title", text: "Проекты" });
hero.createEl("p", { cls: "ud-note-sub", text: "Здесь собраны все рабочие и личные проекты. Поиск и навигация работают прямо внутри страницы без лишних визуальных эффектов." });
