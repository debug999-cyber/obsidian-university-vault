> Map of Content для предмета «Физра». Автоматически собирает все заметки.

```dataviewjs
const pages = dv.pages('"10-Университет/Физра"')
...
for (const [type, items] of Object.entries(byType)) {
    ...
    header.createEl("span", { text: type, cls: "ud-section-title" }).style.textTransform = "uppercase";
