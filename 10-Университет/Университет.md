...
const pages = dv.pages('"10-Университет"')
    .where(p => !p.file.name.startsWith("_Index_of_") && !p.file.name.startsWith("MOC") && p.file.name != "Университет")
    .sort(p => p.file.name);

const byFolder = {};
for (const p of pages) {
    const key = p.file.folder.replace("10-Университет/", "").split("/")[0] || "Разное";
