...
> Map of Content для предмета «Биология». Автоматически собирает все заметки.
...
const pages = dv.pages()
    .where(p => p.file.path.startsWith("10-Университет/Биология"))
    .where(p => p.file.name != "MOC — Биология")
    .sort(p => p.file.name)
    .array();
