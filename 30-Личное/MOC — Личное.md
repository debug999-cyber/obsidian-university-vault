...
> Map of Content для предмета «Личное». Автоматически собирает все заметки.
...
const pages = dv.pages()
    .where(p => p.file.path.startsWith("30-Личное"))
    .where(p => p.file.name != "MOC — Личное")
    .sort(p => p.file.name)
    .array();
...
## Связанные заметки
- [[2026-05-24 — Почта и пароль от стим]]
- [[Без названия]]
