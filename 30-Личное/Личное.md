...
const pages = dv.pages('"30-Личное"')
  .where(page => isContentPage(page, ["Личное"]))
  .array()
  .sort((a, b) => a.file.name.localeCompare(b.file.name, "ru"));

const groupsMap = new Map();
for (const item of pages) {
  const rel = (item.file.folder || "").replace("30-Личное/", "");
...
hero.createEl("div", { cls: "ud-note-eyebrow", text: "Личный раздел" });
hero.createEl("h1", { cls: "ud-note-title", text: "Личное" });
hero.createEl("p", { cls: "ud-note-sub", text: "Раздел для личных заметок и материалов. Карточки и группировка остаются в том же визуальном стиле, что и рабочий стол." });
